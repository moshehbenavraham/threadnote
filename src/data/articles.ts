export interface Article {
  ...
  readTime: string;
  /** Topic labels used for filtering on the index page. Order matters: the first tag is treated as the primary topic. */
  tags: string[];
  content?: string;
}

/**
 * The canonical, ordered set of topics shown as filter chips on the home page.
 * Sourced from the `tags` field on each article; defined explicitly so the
 * order is deterministic and matches editorial priorities (not alphabetical).
 */
export const TOPICS = [
  "Wardrobe",
  "Sustainability",
...
/** Returns the list of topics that have at least one article, preserving the TOPICS order. */
export const getActiveTopics = (): Topic[] => {
  const used = new Set<string>();
  for (const article of articles) {
    for (const tag of article.tags) used.add(tag);
  }
  return TOPICS.filter((topic) => used.has(topic));
};
